...
class C4AI:
    ...
    def SaveWeights(self, fileName):
        #Concatenate fileName with weights path
        w1FileName = self.weightsPath + fileName + '1.npy'
        w2FileName = self.weightsPath + fileName + '2.npy'

        try:
            f1 = open(w1FileName, 'w')
            f2 = open(w2FileName, 'w')
    ...
    def LoadWeights(self, fileName):
        # Concatenate fileName with weights path
        w1FileName = self.weightsPath + fileName + '1'
        w2FileName = self.weightsPath + fileName + '2'

        try:
            self.w1 = np.load(w1FileName)
            self.w2 = np.load(w2FileName)
        except:
            print("Unexpected error:", sys.exc_info()[0])
    ...
    def evaluateMoves(self, CurrentState, NextMoves, CurPlayerColor, OpponentColor, CurPlayerWin, OpponentWin, BoardFull, PlayerName="P1"):
        ...
        if CurPlayerWin:
            ...
            self.w1 = dJdW2 * self.learnRate
            self.w2 = dJdW1 * self.learnRate

            'Save weights'
            self.SaveWeights(PlayerName + 'Weights')
        elif OpponentWin:
            self.reward = -1
        elif BoardFull:
            self.reward = 0
